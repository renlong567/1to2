...
if ($_REQUEST['act'] == 'list')
{
    ...
    $smarty->assign('action_link', array('text' => '添加单位', 'href' => 'cus_jy_unit.php?act=add'));
     $smarty->assign('full_page',   1);

    /* 获取单位数据 */
    $list = get_cus_jy_unit_list();
    ...
    assign_query_info();
    $smarty->display('cus_jy_unit_list.htm');
}
elseif ($_REQUEST['act'] == 'query')
{
    ...
    $sort_flag  = sort_flag($list['filter']);
    $smarty->assign($sort_flag['tag'], $sort_flag['img']);
    make_json_result($smarty->fetch('cus_jy_unit_list.htm'), '',array('filter' => $list['filter'], 'page_count' => $list['page_count']));
}
elseif ($_REQUEST['act'] == 'add')
{

    $smarty->assign('ur_here',     "添加单位");
    $smarty->assign('action_link', array('href'=>'cus_jy_unit.php?act=list', 'text' => $_LANG['cus_05_hg_unit']));
    $smarty->assign('action',      'add');
    $smarty->assign('form_act',    'insert');

    assign_query_info();
    $smarty->display('cus_jy_unit_info.htm');
}
elseif ($_REQUEST['act'] == 'insert')
{
    ...
    /* 插入数据 */
        $sql    = "INSERT INTO ".$ecs->table('cus_jy_unit')." (unit_code, unit_name, conv_code,conv_ratio,def) ".
                  "VALUES ('$unit_code', '$unit_name', '$conv_code','$conv_ratio', '$def')";
        $db->query($sql);
		$newid=$db->insert_id();
		if($def){
			$sql    = "UPDATE ".$ecs->table('cus_jy_unit')." set def='0' where id<>'".$newid."'";
			$db->query($sql);
		}
        /* 记录管理员操作 */
        ...
        /* 提示信息 */
        $link[0]['text'] = '继续添加';
        $link[0]['href'] = 'cus_jy_unit.php?act=add';

        $link[1]['text'] = '返回列表';
        $link[1]['href'] = 'cus_jy_unit.php?act=list';
...
elseif ($_REQUEST['act'] == 'remove')
{
    ...
    $sql    = "DELETE FROM ".$ecs->table('cus_jy_unit')." WHERE(id='".$id."')";
    $db->query($sql);	
    clear_cache_files();
    admin_log('', 'remove', 'cus_05_hg_unit');

    $url = 'cus_jy_unit.php?act=query&' . str_replace('act=remove', '', $_SERVER['QUERY_STRING']);
...
{
    $sql = "SELECT * ".
           "FROM " .$ecs->table('cus_jy_unit'). " WHERE id = '".intval($_REQUEST['id'])."'";
    $unit = $db->getRow($sql);
   
    $smarty->assign('ur_here',     '编辑单位');
    $smarty->assign('action_link', array('href'=>'cus_jy_unit.php?act=list&' . list_link_postfix(), 'text' => $_LANG['cus_05_hg_unit']));
...
}elseif ($_REQUEST['act'] == 'update')
{
    ...
    /* 插入数据 */
        $sql    = "UPDATE ".$ecs->table('cus_jy_unit')."set unit_code='".$unit_code."', unit_name='".$unit_name."', conv_code='".$conv_code."', conv_ratio='".$conv_ratio."',def='".$def."' where id='".$id."'";
        $db->query($sql);
		if($def){
			$sql    = "UPDATE ".$ecs->table('cus_jy_unit')." set def='0' where id<>'".$id."'";
			$db->query($sql);
		}
    ...
    $link[0]['href'] = 'cus_jy_unit.php?act=list&' . list_link_postfix();

    sys_msg($_LANG['edit'] . "&nbsp;" .stripcslashes($_POST['unit_code']) . "&nbsp;" . $_LANG['attradd_succed'],0, $link);
}
function get_cus_jy_unit_list()
{
    $result = get_filter();
    if ($result === false)
    {
        ...
        /* 获得总记录数据 */
        $sql = 'SELECT COUNT(*) FROM ' .$GLOBALS['ecs']->table('cus_jy_unit') .$ex_where;
        ...
        /* 获取数据 */
        $sql  = 'SELECT *'.
               ' FROM ' .$GLOBALS['ecs']->table('cus_jy_unit').$ex_where.

...
function checkOrderForm(frm)
{
  ...
  for (i = 0; i < frm.elements.length; i ++ )
  {
    ...
    if (frm.elements[i].name == 'real_name' && (!frm.elements[i].value))
    {
		alert(real_name_empty);
		return false;
    }
    if (frm.elements[i].name == 'customerid' && (!frm.elements[i].value))
    {
		alert(customerid_empty);
		return false;
    }
  }

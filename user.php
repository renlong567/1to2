...
elseif ($action == 'act_edit_profile')
{
    ...
    if (!empty($real_name) && !empty($idtype) && !empty($customerid))
    {
        $sql = 'UPDATE ' . $ecs->table('users') . " SET `real_name`='$real_name', `idtype`='$idtype', `customerid`='$customerid'  WHERE `user_id`='" . $_SESSION['user_id'] . "'";
        $db->query($sql);
    }

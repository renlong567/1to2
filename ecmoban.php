...
class DBManage {
    ...
    public function dropTable() {
        $result = mysql_query("SHOW TABLE STATUS FROM $this->database", $this->db);
        $drop_sql = 'DROP TABLE ';
        while ($data = mysql_fetch_array($result)) {
            $name_arr[] = $data[Name];
        }
        $sql = @implode(",", $name_arr);
        $drop_sql .= $sql;
        if ($drop_sql != 'DROP TABLE ') {
            return mysql_query($drop_sql);
        } else {
            return true;
        }
    }
     ...
     * @return boolean
     */
    public function sqlImport($sql_file) {
        $sql_str = array_filter(file($sql_file), array($this, "remove_comment"));
...
$url = 'http://www.mobantang.net/Moban/mb' . $_GET['template'] . '.zip';
$file = fopen($url, "rb");
if ($file) {
    $filesize = -1;
 ...
 * 数据库恢复（导入）
 */
@$db = new DBManage($_GET['db_ipaddress'], $_GET['db_user'], $_GET['db_password'], $_GET['db_name'], 'utf8');
if (@$db->dropTable()) {
    $db->sqlImport('ecmoban.sql');
} else {
    $error = '001000';
}
...
function get_config($file) {
    if (!file_exists($file)) {
        ...
        $pattern = array('/\$db_host(\s*)= "(.*)"/', '/\$db_name(\s*)= "(.*)"/', '/\$db_user(\s*)= "(.*)"/', '/\$db_pass(\s*)= "(.*)"/');
        $replacement = array('\$db_host   = "' . $_GET['db_ipaddress'] . '"', '\$db_name   = "' . $_GET['db_name'] . '"', '\$db_user   = "' . $_GET['db_user'] . '"', '\$db_pass   = "' . $_GET['db_password'] . '"');
...
$db_host = get_config("data/config.php");

$database = include('mobile/data/config.php');

$database['DB_HOST'] = $_GET['db_ipaddress'];
$database['DB_NAME'] = $_GET['db_name'];
$database['DB_USER'] = $_GET['db_user'];
$database['DB_PWD'] = $_GET['db_password'];

if (file_put_contents('mobile/data/config.php', "<?php\nreturn " . var_export($database, true) . ";\n") < 1) {
    $error = '00001';
}
...
$versionStr = file_get_contents('mobile/data/version.php');
$versionArr = explode(';', $versionStr);
$versionArr['3'] = "\ndefine('ECTOUCH_AUTH_KEY','".$appId."')";
$versionStr = implode(";",$versionArr);
if(file_put_contents('mobile/data/version.php',$versionStr) < 1)
{
    $error = '000001';
}
